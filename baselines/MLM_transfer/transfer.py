...
def main():
    ...
    if task_name == "yelp" or task_name == "jigsaw":
        if step == "bert_st":
            ...
            bert_name = "{}/BertForMaskedLM_{}_wo_label_epoch10".format(task_name.lower(), task_name.lower())
            model = load_model(bert_name)
        elif step == "bert_ft_cls":
            bert_name = "{}/BertForMaskedLM_{}_wo_label_w_cls_epoch10".format(task_name.lower(), task_name.lower())
            model = load_model(bert_name)
    # model.cuda()
    model.eval()
...
def run_transfer(model, tokenizer, task_name, epoch=None, model_name=None, modified="", set="test"):
    ...
    for example in train:
        ...
        predictions = model(ids_tensor, segment_tensors)
